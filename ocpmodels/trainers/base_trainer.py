...
class BaseTrainer(ABC):
    ...
    def load_loss(self) -> None:
        ...
        #     "energy": self.config["optim"].get("loss_energy", val),
        # }
        self.loss_fn: Dict[str, str] = {
            "energy": self.config["optim"].get("loss_energy", "Huber_loss"),
            "force": self.config["optim"].get("loss_force", "Huber_loss"),
        }
        for loss, loss_name in self.loss_fn.items():
            if loss_name in ["l1", "mae"]:
                ...
                self.loss_fn[loss] = nn.HuberLoss(delta=0.1)
            else:
